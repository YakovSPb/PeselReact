...
// export const fetchBreeds = (currentPage:number) => useQuery(
//     ['breeds', currentPage],
//     () => instance.get(`breeds?fields=id,name`))
...
//
//     async fetchDogs(typeImg, pageSize, currentPage, favoriteDogs) {
//         let dogs = await instance.get(`images/search?mime_types=${typeImg ? 'jpg,png' : 'gif'}&limit=${pageSize}&page=${currentPage}&order=Desc`)
//         return dogs.data.map(dog => {
//             let favoriteDog = favoriteDogs.find(fDog => fDog.image_id === dog.id)
...
//
//     async fetchFavorite() {
//         return await instance.get('favourites', { params: { order: 'DESC' } })
//     },
//     async fetchDogById(idDog, favoriteDogArr = []) {
//         const dogById = await instance.get(`images/` + idDog)
//         let dogProfile = dogById?.data;
...
//         }
//         const find = favoriteDogArr.find(fDog => fDog.image_id === dogProfile.image_id)
//         if (find != undefined) {
...
//         return dogProfile;
//     },
//     async saveFavorite(image_id){
//         return await instance.post('favourites', { image_id, sub_id: "User-123" })
//     }
// })
//
